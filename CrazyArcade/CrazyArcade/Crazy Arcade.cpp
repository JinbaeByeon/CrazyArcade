...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM  lParam)
{
	...
	case WM_CREATE:
		...
		// 비트맵 로드
		SetBitmap();
		SetTimer(hwnd, Bubble_BfBoom, 100, TimeProc_Bubble_BfBoom);   // 물풍선 애니메이션
		SetTimer(hwnd, Monster_Move, 50, TimeProc_Monster_Move);
		P1_Speed = 35;
		P2_Speed = 35;
		P1_bCount = 1;
		P2_bCount = 1;
	...
	case WM_LBUTTONDOWN:
		...
		if (GameState == ROBBY)
		{
			if (Collision(GameStart, M_X, M_Y))
			{
				...
				TextOn = TRUE;
				SetTimer(hWnd, 8, 750, TimeProc_Text);
				GameState = INGAME;
				bSceneChange = true;
				if (SelectMap1)
					Sel_Map = 0;
...
void Animation()
{
	...
	if (GameState == INGAME)
	{
			...
			if (P1_Bubble_Flow[i])
			{
				...
				for (int j = 0; j <= P1_Power; ++j) {   // 위
					...
					if (Collision(Player1, (Tile_Bubble1[i].left + Tile_Bubble1[i].right) / 2, Tile_Bubble1[i].top - 40 * j) ||
						...
						P1_Speed = 100;
						SetTimer(hwnd, In_Bubble, P1_Speed, TimeProc_InBubble);
						P1_InBubble = TRUE;
					}
			...
			if (P2_Bubble_Flow[i])
			{
				...
				for (int j = 0; j <= P2_Power; ++j) {   // 위
					...
					if (Collision(Player2, (Tile_Bubble2[i].left + Tile_Bubble2[i].right) / 2, Tile_Bubble2[i].top - 40 * j) ||
						...
						P2_Speed = 100;
						SetTimer(hwnd, In_Bubble, P2_Speed, TimeProc_InBubble);
						P2_InBubble = TRUE;
					}
				...
				if (Itemset[Sel_Map][i][j] != 0 && Itemset[Sel_Map][i][j] != 7 && Itemset[Sel_Map][i][j] != Pint) {
					if (IntersectRect(&Temp, &Player1, &Tile[i][j])) {
						...
						if (Itemset[Sel_Map][i][j] == Speed) {
							...
							if (P1_Speed >= 20)
								P1_Speed -= 5;
							SetTimer(hwnd, P1, P1_Speed, TimeProc_P1_Move);
						}
					...
					if (IntersectRect(&Temp, &Player2, &Tile[i][j])) {
						...
						if (Itemset[Sel_Map][i][j] == OnePower) {
							...
							if (P2_Speed >= 20)
								P2_Speed -= 5;
							SetTimer(hwnd, P2, P2_Speed, TimeProc_P2_Move);
						}
		...
		if (P1_Die && P2_Die&&Ending) {
			...
			SetTimer(hwnd, 8, 100, TimeProc_Text);
		}
...
void CALLBACK TimeProc_Bubble_BfBoom(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	...
	for (int i = 0; i < 7; i++) {
		if (P1_Bubble_Boom[i]) {
			...
			SetTimer(hwnd, Bubble_Flow, 100, TimeProc_Bubble_Flow);
		}
		if (P2_Bubble_Boom[i]) {
			P2_Bubble[i] = FALSE;
...
void CALLBACK TimeProc_InBubble(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	if (P1_InBubble)
	{
		...
		else if (P1_BubbleCount == 33)
		{
			...
			SetTimer(hwnd, Die, 150, TimeProc_Die);
		}
		else
		{
